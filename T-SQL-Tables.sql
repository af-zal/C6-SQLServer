...
employee_id INT PRIMARY KEY, first_name VARCHAR(50), last_name VARCHAR(50), hire_date DATE, department_id INT
);
...
UPDATE employees SET first_name = 'Jane'
WHERE employee_id = 1;

DELETE FROM employees
WHERE employee_id = 1;

--Delete from employees where condition, can be rolled back
...
--WHERE and LIKE
select * from tblEmployee
where [EmployeeLastName] <> 'Word' --not equal to

select * from tblEmployee
where [EmployeeLastName] like '_W%' --starts with W and has atleast 1 more char in its name
...
select * from tblEmployee
where EmployeeNumber in (200, 204, 131)
